...
export default function TileGrid({ tiles, selectedIdxs, flashIdxs, hintedIdxs, onPick }) {
  return (
    <div className="grid grid-cols-4 grid-rows-2 gap-2 w-full">
      {tiles.map((t, i) => {
        ...
        const isHinted = hintedIdxs.includes(i);
        return (
          <div
            key={i}
            className={`${TILE_BANK} ${
              isHinted
                ? 'border-coral text-coral-deep shadow-[3px_4px_0_#E8654A]'
                : 'border-charcoal text-charcoal shadow-[3px_4px_0_#1a1a1a]'
            } active:translate-x-[3px] active:translate-y-[4px] active:shadow-none ${isSelected ? 'opacity-30 pointer-events-none' : ''} ${isFlashing ? 'animate-flash' : ''}`}
            onClick={() => onPick(i)}
          >
            {t}
          </div>
